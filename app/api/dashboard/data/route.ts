...
function percentChange(current: number, previous: number): number {
  if (previous === 0) {
    // If we had no previous data:
    // - still 0 -> 0%
    // - some activity now -> 100% (a "from zero" jump)
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const ordersYesterday = await prisma.transaction.count({
      where: {
        userId: session.sub,
        status: { in: DISPLAY_STATUSES as any },
    ...
    const yesterdaySpendingAgg = await prisma.transaction.aggregate({
      where: {
        userId: session.sub,
        status: { in: DISPLAY_STATUSES as any },
        createdAt: { gte: yesterday, lt: today },
    ...
    const yesterdayRevenue =
      yesterdaySpendingAgg._sum.grossAmount ??
      yesterdaySpendingAgg._sum.price ??
      0;

    const ordersTodayChangePct = Math.round(percentChange(ordersToday, ordersYesterday) * 10) / 10;
    const revenueTodayChangePct = Math.round(percentChange(todayRevenue, yesterdayRevenue) * 10) / 10;
