...
export default function SettingsPage() {
  ...
  const [settings, setSettings] = useState<AppSettings>({
    ...
    appearance: { theme: "dark", language: "id", compactMode: false },
  ...
  const tabs = [
    ...
    { id: "notifications" as const, label: "Notifikasi", icon: Icons.bell },
    { id: "appearance" as const, label: "Tampilan", icon: Icons.palette },
    { id: "privacy" as const, label: "Privasi", icon: Icons.shield },
  ];
  ...
  const handleSave = async () => {
    ...
    if (!profile.name || profile.name.trim().length < 2) {
      showAlert("Nama minimal 2 karakter", { variant: "error" });
      return;
    }

    // Validasi phone jika diisi
    if (profile.phone && profile.phone.length > 0 && profile.phone.length < 10) {
      showAlert("Nomor telepon minimal 10 digit", { variant: "error" });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        showAlert("Profil berhasil disimpan!", { variant: "success" });
      } else {
        throw new Error(data.message || "Gagal menyimpan");
      }
    } catch (err) {
      showAlert(err instanceof Error ? err.message : "Gagal menyimpan pengaturan", { variant: "error" });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDeleteAccount = async () => {
    const confirmed = await confirm({
      title: "Hapus Akun?",
      message: "Semua data Anda akan dihapus permanen dan tidak dapat dikembalikan. Ketik 'HAPUS' untuk konfirmasi.",
    ...
    if (confirmed) {
      ...
      if (doubleConfirm) {
        ...
        try {
          ...
          if (res.ok) {
            showAlert("Akun berhasil dihapus. Anda akan dialihkan...", { variant: "success" });
          ...
          showAlert(err instanceof Error ? err.message : "Gagal menghapus akun", { variant: "error" });
        } finally {
          setIsDeleting(false);
        }
      }
        ...
        if (!user) {
          return (
            <div className="matcha-empty">
              ...
              <p className="matcha-empty__text">Silakan login untuk mengakses pengaturan.</p>
...
function ProfileSettings({ profile, setProfile }: { profile: UserProfile; setProfile: (p: UserProfile) => void }) {
  ...
  return (
    <>
      <SettingsSection icon={Icons.user} title="Informasi Profil" description="Kelola informasi dasar akun Anda">
        <div className="profile-avatar-section">
          ...
          <div className="profile-avatar-actions">
            ...
            >
              ...
              <span>{isUploadingPhoto ? "Mengupload..." : "Ganti Foto"}</span>
            </button>
            <span className="photo-hint">JPG, PNG. Maks 2MB</span>
          </div>
        </div>
        <div className="settings-form">
          <div className="matcha-input-group">
            <label className="matcha-input-label">Nama Lengkap</label>
            <input
              type="text"
          ...
          <div className="matcha-input-group">
            ...
            <span className="matcha-input-hint">Email tidak dapat diubah</span>
          </div>
          <div className="matcha-input-group">
            <label className="matcha-input-label">Nomor Telepon</label>
            <input
              type="tel"
              className="matcha-input"
              value={profile.phone || ""}
              ...
              maxLength={15}
            />
            <span className="matcha-input-hint">Contoh: +6281234567890</span>
          </div>
        </div>
      </SettingsSection>
...
function NotificationSettings({ settings, setSettings }: { settings: AppSettings; setSettings: (s: AppSettings) => void }) {
  ...
  return (
    <SettingsSection icon={Icons.bell} title="Notifikasi" description="Kelola preferensi notifikasi Anda">
      <SettingsRow label="Email Notifikasi" description="Terima notifikasi melalui email">
        <Toggle checked={settings.notifications.email} onChange={(v) => update("email", v)} />
      </SettingsRow>
      <SettingsRow label="Push Notifikasi" description="Terima notifikasi push di browser">
        <Toggle checked={settings.notifications.push} onChange={(v) => update("push", v)} />
      </SettingsRow>
      <SettingsRow label="Update Order" description="Notifikasi status pesanan">
        <Toggle checked={settings.notifications.orderUpdates} onChange={(v) => update("orderUpdates", v)} />
      </SettingsRow>
      <SettingsRow label="Promo & Penawaran" description="Terima info promo terbaru">
...
function AppearanceSettings({ settings, setSettings }: { settings: AppSettings; setSettings: (s: AppSettings) => void }) {
  ...
  return (
    <SettingsSection icon={Icons.palette} title="Tampilan" description="Sesuaikan tampilan aplikasi">
      <div className="theme-selector">
        <label className="matcha-input-label">Tema</label>
...
function PrivacySettings({ settings, setSettings, onDeleteAccount, isDeleting }: {
  ...
  return (
    <>
      <SettingsSection icon={Icons.shield} title="Privasi" description="Kelola pengaturan privasi akun">
        <SettingsRow label="Tampilkan Profil" description="Izinkan orang lain melihat profil Anda">
          <Toggle
            checked={settings.privacy.showProfile}
            onChange={(v) => setSettings({ ...settings, privacy: { ...settings.privacy, showProfile: v } })}
          />
        </SettingsRow>
        <SettingsRow label="Bagikan Analytics" description="Bantu kami meningkatkan layanan">
          <Toggle
      ...
      <SettingsSection icon={Icons.trash} title="Zona Bahaya" description="Tindakan yang tidak dapat dibatalkan">
        <div className="danger-zone">
          <div className="danger-zone__info">
            <h4>Hapus Akun</h4>
            <p>Setelah dihapus, semua data Anda akan hilang permanen.</p>
          </div>
          <button className="matcha-btn matcha-btn--danger" onClick={onDeleteAccount} disabled={isDeleting}>
            {isDeleting ? (
              ...
              <>
                {Icons.trash}<span>Hapus Akun</span>
              </>
            )}
          </button>
        </div>
