          ...
          <div>
            ...
            <div className="export-health">
              ...
              <button
                ...
                onClick={async () => {
                  if (!lastAuditErrorPayload) return;
                  const pretty = formatJsonPretty(lastAuditErrorPayload);
                  try {
                    await navigator.clipboard.writeText(pretty);
                    pushToast("Error payload copied", "success");
                  } catch {
                    window.prompt("Copy error payload:", pretty);
                  }
                }}
                disabled={!lastAuditErrorPayload}
                title="Copy last error payload"
              >
                Copy error
              </button>
            </div>

            {showAuditErrorPreview && lastAuditErrorPayload ? (
              <div
                style={{
                  marginTop: 10,
              ...
              >
                {auditErrorPreviewMode === "raw" ? (
                  <pre style={{ margin: 0, whiteSpace: "pre-wrap" }}>{lastAuditErrorPayload}</pre>
                ) : (() => {
                    try {
                      const parsed = JSON.parse(lastAuditErrorPayload) as unknown;
                      return (
                        <JsonTree
                          value={parsed}
                          defaultOpen
                          ...
                          onCopyPath={async (p) => {
                            try {
                              ...
                              pushToast(`Path copied: ${p}`, "success");
                            } catch {
                          ...
                          onCopyValue={async (v) => {
                            try {
                              ...
                              pushToast("Value copied", "success");
                            } catch {
                      ...
                      return <pre style={{ margin: 0, whiteSpace: "pre-wrap" }}>{formatJsonPretty(lastAuditErrorPayload)}</pre>;
                    }
                  })()}
              </div>
            ) : null}
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  if (!res.ok) {
                    const body = (await res.json().catch(() => null)) as unknown;
                    const record = isRecord(body) ? body : null;
                    setError(record?.message ? String(record.message) : "Gagal membuat export job.");
                    return;
                  }
...
// COMMENTED:       {showShortcutsHelp && (
// COMMENTED:         <div
// COMMENTED:           role="dialog"
// COMMENTED:           aria-modal="true"
// COMMENTED:           aria-label="Keyboard shortcuts"
// COMMENTED:           className="modal-overlay"
// COMMENTED:           onClick={() => setShowShortcutsHelp(false)}
// COMMENTED:         >
// COMMENTED:           <div className="card modal-card" onClick={(e) => e.stopPropagation()}>
// COMMENTED:             <div className="card-header modal-header">
// COMMENTED:               <h4 style={{ margin: 0 }}>Keyboard shortcuts</h4>
// COMMENTED:               <button
// COMMENTED:                 className="secondary-btn secondary-btn--sm"
// COMMENTED:                 onClick={() => setShowShortcutsHelp(false)}
// COMMENTED:                 title="Close"
// COMMENTED:               >
// COMMENTED:                 Close
// COMMENTED:               </button>
// COMMENTED:             </div>
// COMMENTED:             <div style={{ padding: 12, fontSize: 13 }}>
// COMMENTED:               <p style={{ margin: 0, opacity: 0.8 }}>
// COMMENTED:                 Shortcuts helper sedang dinonaktifkan sementara untuk merapihkan parsing/TypeScript.
// COMMENTED:                 Akan diaktifkan lagi setelah refactor.
// COMMENTED:               </p>
// COMMENTED:             </div>
// COMMENTED:           </div>
// COMMENTED:         </div>
// COMMENTED:       )}
// COMMENTED: 
// COMMENTED:       {jobDetailId ? (
// COMMENTED:         <div
// COMMENTED:           role="dialog"
// COMMENTED:           aria-modal="true"
// COMMENTED:           aria-label="Export job detail"
// COMMENTED:           onClick={() => {
// COMMENTED:             const top = getTopOverlayKey();
// COMMENTED:             if (top && top !== exportJobDrawerKeyRef.current) return;
// COMMENTED:             setJobDetailId(null);
            setJobDetail(null);
          }}
          style={{
            position: "fixed",
