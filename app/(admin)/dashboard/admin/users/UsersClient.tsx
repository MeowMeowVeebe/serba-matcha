...
export default function UsersClient({ initial }: { initial: AdminUsersInitialData }) {
  ...
  const saveRoles = async (userId: string) => {
    ...
    try {
      ...
      if (!ok) {
        showAlert("Failed to save user roles.", { variant: "error" });
        return;
      }

      showAlert("User roles saved successfully.", { variant: "success" });
      cancelEdit();
  ...
  const bulkApply = async () => {
    const userIds = Object.entries(bulkSelected)
      ...
      .map(([id]) => id);
    if (!userIds.length) {
      showAlert("Select at least 1 user.", { variant: "warning" });
      return;
    }
   ...
   <div className="card">
     ...
     <div className="onboarding-progress">
       <div>
         ...
         <p className="onboarding-progress__note">7 new users haven't completed profile setup.</p>
       </div>
     ...
     <div className="ramp-up-tracker">
       <div>
         ...
         <p className="ramp-up-tracker__note">3 new users on-track, 2 need mentoring.</p>
       </div>
     ...
     <div className="panel">
       ...
       <p className="helper-text">
         Select users via checkbox in the table, then choose roles here. This action will <b>replace</b> the roles of selected users.
       </p>
     </div>
      ...
      {isLoading ? (
        ...
        <div style={{ marginTop: 12 }}>
          <EmptyState
            title="No users"
            description={query.q ? "Try changing your search keywords." : "No matching users yet."}
            action={
              query.q ? (
