...
export default function DashboardPage() {
  return (
    <AccountShell title="Dashboard" description="Serba Matcha Business Summary" breadcrumbs={[{ label: "Dashboard" }]}>
...
function DashboardContent({ user, isLoadingUser }: { user: any; isLoadingUser: boolean }) {
  ...
  useEffect(() => {
    ...
    // "Real-time" updates: polling + refresh when tab becomes active
  ...
  if (!user) {
    return (
      <div className="matcha-empty">
        ...
        <p className="matcha-empty__text">Please sign in to view the dashboard.</p>
        <Link href="/dashboard/login" className="matcha-btn matcha-btn--primary">
          Sign In
        </Link>
      </div>
    );
  ...
  const metrics = data?.metrics || {
    ordersToday: 0,
    ordersTodayChangePct: 0,
    revenue: 0,
    revenueTodayChangePct: 0,
    topDish: "-",
    pendingOrders: 0,
  };

  const ordersDelta = Number.isFinite(metrics.ordersTodayChangePct as number) ? (metrics.ordersTodayChangePct as number) : 0;
  const revenueDelta = Number.isFinite(metrics.revenueTodayChangePct as number) ? (metrics.revenueTodayChangePct as number) : 0;
    ...
    <div className="dashboard-page">
      <section className="dashboard-welcome animate-fade-in">
        <div className="dashboard-welcome__content">
          ...
          <p>Here is today's Serba Matcha business summary.</p>
        </div>
      ...
      <section className="matcha-grid matcha-grid--4 animate-fade-in animate-delay-1">
        <StatCard
          ...
          trend={ordersDelta === 0 ? "neutral" : ordersDelta > 0 ? "up" : "down"}
          trendValue={`${ordersDelta > 0 ? "+" : ""}${ordersDelta}%`}
        />
        <StatCard
          icon={Icons.spending}
          label="Total Spending"
          value={formatCurrency(metrics.revenue)}
          trend={revenueDelta === 0 ? "neutral" : revenueDelta > 0 ? "up" : "down"}
          trendValue={`${revenueDelta > 0 ? "+" : ""}${revenueDelta}%`}
        />
        ...
        <div className="matcha-card">
          ...
          <div className="matcha-card__body">
            <div className="quick-actions">
              <QuickAction href="/menu" icon={Icons.orders} label="Kunjungi Menu" />
              <QuickAction href="/dashboard/settings" icon={Icons.customers} label="Profile Settings" />
              <QuickAction href="/dashboard/security" icon={Icons.trending} label="Reports" />
            </div>
          </div>
        </div>
      </div>
